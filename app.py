...
st.write("Check if your dream domain is available, or get AI-powered alternatives if it's taken!")

with st.form("domain_check_form"):
    domain_input = st.text_input("Enter a domain name (e.g., myawesomeidea.com)", placeholder="myawesomeidea.com")
...
if submit_button:
    if domain_input:
        ...
        if available:
            ...
            with col1:
                st.link_button("Namecheap", f"https://www.namecheap.com/domains/registration/results/?domain={domain}", use_container_width=True)
            with col2:
                st.link_button("GoDaddy", f"https://www.godaddy.com/domainsearch/find?checkAvail=1&tmskey=&domainToCheck={domain}", use_container_width=True)
            with col3:
                st.link_button("Squarespace", f"https://domains.squarespace.com/domain-search", use_container_width=True)
            ...
            with st.spinner("Generating creative alternatives with Gemini..."):
                alternatives = get_alternative_domains(domain)
            ...
            if isinstance(alternatives, list):
                ...
                while available_count < 4 and retry_count < max_retries:
                    ...
                    new_alternatives = get_alternative_domains(domain, excluded_domains=list(all_suggestions.keys()))
                ...
                for alt_domain in available_list:
                    if displayed_available < 4:
                        ...
                        col1, col2, col3 = st.columns(3)
                        with col1:
                            st.link_button("Namecheap", f"https://www.namecheap.com/domains/registration/results/?domain={alt_domain}", use_container_width=True)
                        with col2:
                            st.link_button("GoDaddy", f"https://www.godaddy.com/domainsearch/find?checkAvail=1&tmskey=&domainToCheck={alt_domain}", use_container_width=True)
                        with col3:
                            st.link_button("Squarespace", f"https://domains.squarespace.com/domain-search", use_container_width=True)
                        st.divider()
